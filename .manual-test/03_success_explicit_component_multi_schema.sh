...
# Scenario: schema has multiple components and --component is provided.
...
SCHEMA_PATH="$SCRIPT_DIR/schema.with-component.json"
...
{"resources":{"pkg:index:Thing":{"isComponent":true},"pkg:index:Other":{"isComponent":true},"pkg:index:Skip":{"isComponent":false}}}
JSON

STDOUT_PATH="$SCRIPT_DIR/out.component-selected.stdout.txt"
STDERR_PATH="$SCRIPT_DIR/out.component-selected.stderr.txt"

set +e
"$BIN_PATH" --in "$SCHEMA_PATH" --component "pkg:index:Thing" >"$STDOUT_PATH" 2>"$STDERR_PATH"
EXIT_CODE=$?
set -e
